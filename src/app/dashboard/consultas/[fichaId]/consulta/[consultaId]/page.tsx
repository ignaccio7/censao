...
export default async function DetalleConsultaPage({
  ...
  params: Promise<{ fichaId: string; consultaId: string }>
}) {
  const { fichaId, consultaId } = await params
  ...
  if (!consulta) {
    return (
      <div className='p-8 text-center text-red-500 bg-red-50 rounded-xl border border-red-100'>
        ...
        <Link
          href={`/dashboard/consultas/${fichaId}`}
    ...
    <main className='pb-8 animate-fade-in space-y-6'>
      ...
      <div className='flex items-center gap-2'>
        {esSeguimiento && consulta.consulta_padre ? (
          // Si es seguimiento → volver a la consulta padre
          <Link
            href={`/dashboard/consultas/${fichaId}/consulta/${consulta.consulta_padre.id}`}
      ...
      <div className='bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden'>
        ...
        {/* Cuerpo: motivo + observaciones + acciones */}
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-gray-100'>
          ...
          <div className='lg:col-span-2 p-5 space-y-4 text-step-1'>
            ...
            {/* Nota: sin sub-seguimientos si ya es seguimiento */}
            {esSeguimiento && (
      ...
      {/* ── Seguimientos (solo en consultas raíz) ──────────────── */}
      {!esSeguimiento && (
                ...
                >
                  ...
                  >
                    ...
                    <div className='flex items-center gap-3'>
                      ...
                      <Link
                        href={`/dashboard/consultas/${fichaId}/consulta/${seg.id}`}
